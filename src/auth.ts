...
// AuthStore 和AuthParser之间应通过共同约定的AuthMetaDataCtx进行通信，AuthMetaDataCtx中存储如Cookie有效时间等，AuthParser根据AuthMetaDataCtx中的信息进行设置


export type AuthStore<UserData, Credit> = {
...
export type AuthConfig<UserData, Credit> = {
  authUserDataCtx: AuthUserDataCtx<UserData>
  autoSave: boolean
  authParser: AuthParser<Credit>
  authStore: AuthStore<UserData, Credit>
}
// 创建一个独立的 Context 来存储修改状态（请求级隔离）
const modifiedStateCtx = createContext<boolean>(false)

export type AuthUserDataCtx<D> = Context<D|undefined> & {
...
export const createAuthCtx = <D>(defaultData: D): AuthUserDataCtx<D> => {
  ...
  const proxyCtx = new Proxy({
    ...
    saveToStore: async () => {
      throw new Error(
        'saveToStore is not implemented yet,You need pass the authUserDataCtx to the middleware to create it ',
      )
    },
    regenerate: async () => {
      throw new Error(
        'regenerate is not implemented yet,You need pass the authUserDataCtx function to the middleware to create it ',
      )
    },
    destroy: async () => {
      throw new Error(
        'destroy is not implemented yet,You need pass the authUserDataCtx function to the middleware to create it ',
      )
    },
  }, {
    get(target, prop) {
  ...
  }) as AuthUserDataCtx<D>
  
  return proxyCtx
}

export const authHeaderCtx = createContext<Response[]>([])

// 辅助类型：从配置中提取类型
export type InferUserData<T> = T extends AuthConfig<infer U, any> ? U : never
export type InferCredit<T> = T extends AuthConfig<any, infer C> ? C : never

// 改进的类型推导版本
export const createFarrowAuth = <
  TUserData = any,
  TCredit = any,
  TConfig extends AuthConfig<TUserData, TCredit> = AuthConfig<TUserData, TCredit>
>(
  config: TConfig
): Middleware<RequestInfo, MaybeAsyncResponse> => {
  const { authParser, authStore, autoSave, authUserDataCtx } = config
  const middleware: Middleware<RequestInfo, MaybeAsyncResponse> = async (request, next) => {
    ...
    const unverifiedUserCredentials = await authParser.get(request)
    
    const createNewAuth=async()=>{
      const createResult = await authStore.create()
      if (!createResult) return Response.json({ error: 'Internal Server Error' }).status(500)
      authUserDataCtx.set(createResult)
      const authHeader = await authParser.set()
      authHeaderCtx.set([...authHeaderCtx.get(), authHeader])
    ...
    if (!unverifiedUserCredentials) {
      ...
      // 清除旧userCredentials
      const authHeader=await authParser.remove()
      authHeaderCtx.set([...authHeaderCtx.get(), authHeader])
      // 如果需要创建，创建新的userCredentials
      if (authStore.autoCreateOnMissing) {
      ...
      const getUserDataResult = await authStore.get(unverifiedUserCredentials)
      ...
      if (getUserDataResult === null) {
        ...
        // 如果结果存在，代表userCredrential有效,存在userData设置authUserData
        const authUserData = getUserDataResult
        authUserDataCtx.set(authUserData)
      }
    }
    ...
    // 如果自动保存
    if (autoSave) {
      const authUserData = authUserDataCtx.get()
      
      // 只在数据存在且被修改时才保存
      if (authUserData !== undefined && authUserDataCtx.isModified) {
        const setResult = await authStore.set(authUserData)
        if (setResult === undefined) return Response.json({ error: 'Internal Server Error' }).status(500)
        if (setResult === false) return Response.json({ error: 'AuthStore Set Failed' }).status(401)
      }
      // 如果数据没被修改但需要更新过期时间（rolling/renew 模式）
      else if (authUserData !== undefined && !authUserDataCtx.isModified) {
        // 优先使用 touch 方法（如果存在）
        if (authStore.touch) {
          const touchResult = await authStore.touch()
    ...
    const authHeaders = authHeaderCtx.get()
    return Response.merge(...authHeaders).merge(response)
  }
    // 定义auth相关方法
    //
    authUserDataCtx.regenerate = async () => {
      //获取当前的userData
      const userData = authUserDataCtx.get()
      //如果userData不存在，返回false
      if (userData === undefined) return false
      //创建新的信息
      const createResult = await authStore.create(userData)
      //如果创建失败，返回undefined
    ...
    authUserDataCtx.destroy = async () => {
      const authUserData = authUserDataCtx.get()
      // 如果authUserData不存在，返回false
      if (authUserData === undefined) return false
      // 如果authUserData存在，调用Store销毁
      const destroyResult = await authStore.destroy()
      // 如果销毁失败，返回false或者undefined，直接返回false或者undefined
      if (!destroyResult) return destroyResult
      // 如果销毁成功，设置authUserDataCtx为undefined
      authUserDataCtx.set(undefined)
      // 如果销毁成功，设置相应header
      const sessionHeader = await authParser.remove()
      authHeaderCtx.set([...authHeaderCtx.get(), sessionHeader])
      return true
    }

    authUserDataCtx.saveToStore = async () => {
      const authUserData = authUserDataCtx.get()
      // 如果authUserData不存在，返回false
      if (authUserData === undefined) return false
...
export const createAuth = createFarrowAuth
